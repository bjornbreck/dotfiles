...
AsyncValue.prototype = {
     ...
     * available.
     */
    by_: function (callback) {
        // Do we already have data or error?
        if (this.bx_) {
            // invoke the callback immediately
            return callback(this.bv_, this.ay_);
        }

        var callbacks = this.bw_ || (this.bw_ = []);
    ...
    bA_: function (value) {
        ...
        if (value && typeof value.then === "function") {
            ...
            var finalPromise = value.then(function onFulfilled(value) {
                nextTick(asyncValue.bA_.bind(asyncValue, value));
            }, function onRejected(err) {
                nextTick(asyncValue.bz_.bind(asyncValue, err));
            });
