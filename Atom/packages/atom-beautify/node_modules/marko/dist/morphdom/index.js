...
var VElement = require("../runtime/vdom/vdom").al_;
var virtualizeElement = VElement.am_;
var morphAttrs = VElement.an_;
...
function morphdom(parentNode, startNode, endNode, toNode, doc, componentsContext) {
    ...
    function insertVirtualNodeBefore(vNode, key, referenceEl, parentEl, component) {
        var realNode = vNode.ap_(doc);
        insertBefore(realNode, referenceEl, parentEl);

        if (vNode.aq_ === ELEMENT_NODE) {
            if (key) {
                realNode.ag_ = key;
    ...
    function resolveComponentEndNode(startNode, vChild, parentNode) {
        ...
        // a single text node
        var isPrevText = vChild.aq_ === TEXT_NODE;

        while (vChild = vChild.ar_) {
            ...
            var isText = vChild.aq_ === TEXT_NODE;
    ...
    function morphComponent(parentFromNode, component, vComponent) {
        ...
        if (startNode.as_ !== undefined || startNode.B_) {
            startNode = insertBefore(createMarkerComment(), startNode, parentFromNode);
        }

        if (endNode.as_ !== undefined || endNode.i_) {
    ...
    function morphChildren(parentFromNode, startNode, endNode, toNode, component) {
        ...
        outer: while (curToNodeChild) {
            toNextSibling = curToNodeChild.ar_;
            curToNodeType = curToNodeChild.aq_;
            ...
            if (curToNodeType === COMPONENT_NODE) {
                if ((matchingFromComponent = existingComponentLookup[componentForNode.id]) === undefined) {
                    if (isRerenderInBrowser === true) {
                        var firstVChild = curToNodeChild.at_;
                        if (firstVChild) {
                            if (!curFromNodeChild) {
                ...
                continue;
            } else if (curToNodeKey = curToNodeChild.av_) {
                ...
                if (curFromNodeChild) {
                    if (curFromNodeChild !== endNode) {
                        curFromNodeKey = curFromNodeChild.ag_;
                        curVFromNodeChild = curFromNodeChild.ah_;
                ...
                if (curFromNodeKey === curToNodeKey) {
                    ...
                    if ((matchingFromEl = componentForNode.w_[curToNodeKey]) === undefined) {
                        if (isRerenderInBrowser === true && curFromNodeChild && curFromNodeChild.nodeType === ELEMENT_NODE && curFromNodeChild.nodeName === curToNodeChild.ao_) {
                            curVFromNodeChild = virtualizeElement(curFromNodeChild);
                            curFromNodeChild.ag_ = curToNodeKey;
                            morphEl(curFromNodeChild, curVFromNodeChild, curToNodeChild, componentForNode, curToNodeKey);
                            curToNodeChild = toNextSibling;
                        ...
                        if (matchingFromEl.as_ !== undefined) {
                            matchingFromEl.as_ = undefined;
                        }
                        curVFromNodeChild = matchingFromEl.ah_;
                        ...
                        if (compareNodeNames(curVFromNodeChild, curToNodeChild)) {
                            if (fromNextSibling === matchingFromEl) {
                                ...
                                if (toNextSibling && toNextSibling.av_ === curFromNodeKey) {
                                    // Single element swap
            ...
            while (curFromNodeChild && curFromNodeChild !== endNode) {
                ...
                if (curFromNodeType === curToNodeType) {
                    if (curFromNodeType === ELEMENT_NODE) {
                        ...
                        } else if (curFromNodeKey = curVFromNodeChild.av_) {
                        ...
                        if (content == curToNodeChild.aw_) {
                            if (/^F\^/.test(content)) {
                                ...
                                while (curToNodeChild = curToNodeChild.ar_) {
                                    if (curToNodeChild.aw_ === closingContent) {
                                        break;
                                    }
                                }
                                curToNodeChild = curToNodeChild.ar_;
                                curFromNodeChild = curFromNodeChild === endNode ? null : curFromNodeChild.nextSibling;
                                continue outer;
                            }
                        } else {
                            curFromNodeChild.nodeValue = curToNodeChild.aw_;
                        }
                    }
                }
    ...
    function morphEl(fromEl, vFromEl, toEl, component, toElKey) {
        var nodeName = toEl.ao_;

        if (isRerenderInBrowser === true && toElKey) {
            component.w_[toElKey] = fromEl;
        }

        var constId = toEl.ax_;
        if (constId !== undefined && vFromEl.ax_ === constId) {
            return;
        }
    ...
    detachedNodes.forEach(function (node) {
        var detachedFromComponent = node.as_;
